...
export async function GET(request: NextRequest) {
  try {
    ...
    if (skipDateFilter && (startDate || endDate)) {
      console.log(`[CSV Export] Skipping date filter because both mobileStatus and healthIdStatus are "without"`)
    }
    ...
    if ((startDate || endDate) && !skipDateFilter) {
      ...
      console.log(`[CSV Export] Applied date filter:`, whereClause.updatedAt)
    }
